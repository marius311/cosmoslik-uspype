...
!     in one set of parameters and produdes the corresponding output. 
    ...
    program driver
       ...
       character(LEN=Ini_max_string_len) FITSfilename
        ...
        character (LEN=Ini_max_string_len), dimension(1000) :: lines
        integer :: line_number

        do

            do line_number = 1, 1000
                read (*,'(a)',end=400) lines(line_number)
                if (trim(lines(line_number))=="$") exit
            end do

            call Ini_Open_FromLines(DefIni, lines(1:line_number), line_number, .false.)
    ...
    !optional parameters controlling the computation
           ...
           if (global_error_flag/=0) then
            ...
            if (P%want_zdrag) then
                write(*,*) "z_drag =",z_drag
                write(*,*) "rs_drag =",rombint(dsoundda,1d-8,1.0d0/(1.0d0+z_drag), 1d-6)
                write(*,*) "DA_drag =",AngularDiameterDistance(z_drag)
            end if
