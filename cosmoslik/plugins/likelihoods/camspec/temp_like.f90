...
contains
  ...
  subroutine calc_like(zlike,  cell_cmb, cell_fg100, cell_fg143, cell_fg217, cell_fg143x217, &
                       cal0, cal1, cal2, beam_coeffs, nell, nspec, nmodes)

    integer :: i, j, l, ipause,ii,jj, nell, nspec, nmodes
    real*8, dimension(:),  allocatable ::  X_theory, X_f, X_data, X_beam_corr_model, Y 
    real*8, dimension(0:nell) :: cell_cmb, cell_fg100, cell_fg143, cell_fg217, cell_fg143x217
    ...
    do l = lminX(2), lmaxX(2)
       zell = dfloat(l)
       X_f(l - lminX(2) + npt(2)) = cell_fg143(l)
    ...
    do l = lminX(3), lmaxX(3)
       zell = dfloat(l)
       X_f(l - lminX(3) + npt(3) ) = cell_fg217(l)
    ...
    do l = lminX(4), lmaxX(4)
       zell = dfloat(l)
       X_f(l - lminX(4) + npt(4) ) = cell_fg143x217(l)
